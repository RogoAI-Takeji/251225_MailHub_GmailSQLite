...
### v1.0.3 (2026-01-06) - 最新版 ✨

**重要な修正:**
- ✅ **設定保存問題を修正**: exe実行時に設定が正しく保存されるようになりました
- ✅ **SMTP接続エラーを修正**: ポート587（STARTTLS）とポート465（SSL/TLS）の両方に対応
- ✅ **メール送信時のフリーズを解消**: 送信処理をスレッド化し、UI応答性を改善
- ✅ **セキュリティ向上**: デフォルトをテキスト表示に変更（HTMLはボタンクリックで表示）
...
💡 **配置のヒント:**
- **EXEファイル単体で動作します**（追加ファイル不要）
- 初回起動後、exeと同じ場所に `config/` と `storage/` フォルダが自動生成されます
- デスクトップに直接配置してもOK
...
- **デフォルトはテキスト表示**（安全・v1.0.3で改善）
- HTMLメールを見たい場合:
  - 「🌐 HTML版（安全）」ボタン: スクリプト除去版
  - 「🌐💀 HTML版（完全）」ボタン: 警告後に完全版表示
...
現在のバージョン（v1.0.3）はPOP3を使用していますが、**Gmail側のPOP3サポートは継続予定**です。

2026年1月に廃止されるのは：
- ❌ GmailのWeb UIで「他社メールをPOP3で取り込む機能」
  - Gmailが他のメールサービスにアクセスしてメールを取り込む機能

継続するもの：
- ✅ 外部クライアント（Mail Hubなど）からGmailへのPOP3アクセス
  - Mail HubがGmailからメールをダウンロードする機能
...
### メール送信に失敗する（v1.0.3で修正済み）

✅ **確認事項:**
- SMTPポートが正しいか（587 または 465）
- アプリパスワード（16桁）を使用しているか
- v1.0.3以降を使用しているか

**エラー例:**
- `[SSL:WRONG_VERSION_NUMBER]` → v1.0.3で修正済み（ポート判定機能追加）

### 設定が保存されない（v1.0.3で修正済み）

✅ **確認事項:**
- v1.0.3以降を使用しているか
- exeと同じフォルダに `config/` と `storage/` が生成されているか
- 管理者権限ではなく通常実行しているか
...
**v1.0.3の改善:**
- `--hidden-import tkinterweb` を追加（HTML表示の安定化）
- exe実行時の設定保存問題を修正
- SMTPポート判定機能の実装
